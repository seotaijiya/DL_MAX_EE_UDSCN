...
for outer_loop in range(3):
    ...
    P_c_A = 10 ** (3.94)
    P_c_S = 10 ** (3.756)
    tx_max = 10 ** (3.1 + 0.5 * outer_loop)
    CDF_granu = 20

    tx_power_set = all_possible_tx_power(Num_BS, Num_power_level - 1)
    print("tx_power_set = ", tx_power_set.shape)
    ...
    power_CDF = OPT_power.ravel() * tx_max
    power_CDF = power_CDF[power_CDF!=0]
    sorted_power = np.sort(power_CDF)
    opt_granu = int(sorted_power.shape[0] / CDF_granu)
    print(sorted_power[::opt_granu])
    print("")
    ...
    for i in range(1):
        model_SE.fit(log_data, labels, batch_size=batch_size_set, epochs=60, verbose=0)
        ...
        power_CDF = Tx_power_DNN_predict.ravel() * tx_max
        power_CDF = power_CDF[power_CDF != 0]
        sorted_power = np.sort(power_CDF)
        opt_granu = int(sorted_power.shape[0] / CDF_granu)
        print(sorted_power[::opt_granu])
